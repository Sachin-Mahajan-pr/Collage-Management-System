...
def add_timetable(request):
    ...
    if request.method == 'POST':
        ...
        try:
            timetable = TimeTable(subject_id=subject_id, staff_id=staff_id, day=day, start_time=start_time, end_time=end_time)
            timetable.save()
            messages.success(request, "Time Table Added")
...
def edit_timetable(request, timetable_id):
    ...
    if request.method == 'POST':
        ...
        try:
            timetable.save()
            messages.success(request, "Time Table Updated")
            return redirect(reverse('manage_timetable'))
        except Exception as e:
            messages.error(request, "Error: " + str(e))
...
def delete_timetable(request, timetable_id):
    timetable = get_object_or_404(TimeTable, id=timetable_id)
    timetable.delete()
    messages.success(request, "Time Table Deleted")
    return redirect(reverse('manage_timetable'))
...
def add_activity(request):
    if request.method == 'POST':
        ...
        try:
            activity = Activity(name=name, head_name=head_name, description=description, date=date)
            activity.save()
            messages.success(request, "Activity Added")
            return redirect(reverse('manage_activity'))
        except Exception as e:
            messages.error(request, "Error: " + str(e))
    return render(request, "hod_template/add_activity_template.html", {'page_title': 'Add Activity'})
...
def edit_activity(request, activity_id):
    ...
    if request.method == 'POST':
        ...
        activity.head_name = request.POST.get('head_name')
        ...
        try:
            ...
            messages.error(request, "Error: " + str(e))
    return render(request, "hod_template/edit_activity_template.html", {'activity': activity, 'page_title': 'Edit Activity'})
...
def add_fee_structure(request):
    if request.method == 'POST':
        ...
        description = request.POST.get('description')
        try:
            fee = FeeStructure(course_id=course_id, amount=amount, description=description)
            fee.save()
            messages.success(request, "Fee Structure Added")
            return redirect(reverse('manage_fee_structure'))
        except Exception as e:
            messages.error(request, "Error: " + str(e))
    return redirect(reverse('manage_fee_structure'))
...
def student_fee_status(request):
    ...
    context = {
        'payments': payments,
        'page_title': 'Student Fee Transactions'
    }
    return render(request, "hod_template/student_fee_status.html", context)

def toggle_fee_status(request, payment_id):
    ...
    return redirect(reverse('student_fee_status'))
